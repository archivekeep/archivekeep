package org.archivekeep.files.crypto.filesystem
...
class InProgressHandler(
    ...
    fun onStart(dstPath: String) {
        lock.withLock {
            inProgressFiles.update { it + setOf(dstPath) }
            jobActiveOnIdle.updateAndGet { null }?.cancel()
        }
    }

    fun onEnd(dstPath: String) {
        lock.withLock {
            val newValues = inProgressFiles.updateAndGet { it - setOf(dstPath) }
